...
void TAbsFitParticle::setCovMatrix(const TMatrixD* theCovMatrix) {
  // Set the measured covariance matrix in the special
  ...
  if(theCovMatrix==0) {
    ...
    cout << "TAbsFitParticle::setCovMatrix:: " << this->GetName() 
	 << " Covariance matrix has wrong size "
	 << "(" << _nPar << " is " << theCovMatrix->GetNcols() << ")"
	 << endl;
  }

}

void TAbsFitParticle::setCovMatrixCart(const TMatrixD* theCovMatrix) {
  // Set the measured covariance matrix in cartesian coordinates
  ...
  } else if (theCovMatrix->GetNcols() == 4 && theCovMatrix->GetNrows() == 4) {

    // C_special = A*C_cart*AT
    TMatrixD* Ainv = getDerivative();
    TMatrixD A( TMatrixD::kInverted, *Ainv );
    TMatrixD AT(TMatrixD::kTransposed, A);
    TMatrixD AC( A, TMatrixD::kMult, *theCovMatrix );
    TMatrixD ACAT( AC, TMatrixD::kMult, AT );
    delete Ainv;
    setCovMatrix(&ACAT);

  } else {
    cout << "TAbsFitParticle::setCovMatrixCart:: " << this->GetName() 
	 << " Covariance matrix has wrong size "
	 << "(" << _nPar << " is " << theCovMatrix->GetNcols() << ")"
	 << endl;
  }
